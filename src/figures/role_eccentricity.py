"""
This is the script that generates Figure 3 in Wagg et al. (2021).

Original version:

https://github.com/TeamLEGWORK/LEGWORK-paper/blob/d0afa5/src/figures/role_eccentricity.py

Original caption:

An illustration of the effect of eccentricity on the detectability 
of a LISA source. The three sets of points are coloured by their 
eccentricity and each individual point corresponds to a harmonic 
frequency, where its height above the curve gives its SNR. We 
annotate each set of points with its total SNR and overlay the 
LISA sensitivity curve. The dotted vertical lines indicate the 
frequency at which the ma- jority of the gravitational wave signal 
is concentrated.
...
snr2_n = lw.snr.snr_ecc_evolving(
    m_1=m_1,
    m_2=m_2,
    f_orb_i=f_orb,
    ecc=ecc,
    dist=dist,
    harmonics_required=100,
    t_obs=4 * u.yr,
    n_step=1000,
...
fig, ax = lw.visualisation.plot_sensitivity_curve(
    frequency_range=np.logspace(-3.5, 0, 1000) * u.Hz, show=False
)
...
for i in range(len(snr2_n)):
    ...
    ax.annotate(
        r"$\rho={{{:1.0f}}}$".format(snr2_n[i].sum() ** (0.5)),
    ...
    ax.plot(
        [f_harm[max_index].value] * 2,
        [1e-20, y_vals[max_index].value],
        color="grey",
        linestyle="dotted",
        lw=2,
        zorder=0,
